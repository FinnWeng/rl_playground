...
if __name__ == "__main__":
    ...
    input_shape = [None, 210, 160, 1]

    model = RL_model.RL_model(input_shape,seq_size, baseline, batch_size, TD_traj_leng, discount_rate,
    ...
    with tf.Session() as sess:
        ...
        # model_file = tf.train.latest_checkpoint('./A2C/model/')
        ...
        for i_episode in range(int(304766),10000000):
            ...
            for step in range(step_size):
                ...
                action_prob = sess.run(model.prediction_prob,
                                       feed_dict={model.x_holder: observation.reshape([1, 210, 160, 1])})  # generate action
                action_prob += 1e-8
                ...
                if training_step >= 0:
                    ...
                    if len(training_policy_target) == 2 * batch_size or done == 1:
                        ...
                        print("training_p1_st:", training_p1_st_array.shape)
                        ...
                        _, actor_loss_real, critic_loss_real, total_loss_real, advantage_real, summary = sess.run(
                            [model.total_training_op, model.actor_loss, model.critic_loss, model.total_loss, model.advantage, merged_summary_op],
            ...
            save_path = saver.save(sess, "./A2C/breakout_A2C_model.ckpt", global_step=i_episode)

...
class RL_model:
    def __init__(self, input_shape, seq_size=4, baseline=0.0, batch_size=32, filter_num=32, TD_traj_leng=5,
        ...
        self.training_LR =  tf.compat.v1.train.exponential_decay(training_LR,global_step,10000,0.99,staircase=True)
    ...
    def build_net(self):
        ...
        prediction_prob = tf.nn.softmax(logits,axis=1)  # (batch_size,4 prob)
    ...
    def encoder_net(self, enc_x_holder):
        ...
        with tf.variable_scope("encoder", reuse=tf.AUTO_REUSE):
            ...
            with tf.variable_scope("layer_1", reuse=tf.AUTO_REUSE):
                ...
                l1_raw_output = tf.keras.layers.Conv2D(self.filter_num*2, kernel_size=3, strides=1, activation="tanh",
                                                       padding="SAME",
                ...
                l1_H_x, l1_L_x = oct_conv_block(l1_H_x, l1_L_x, channel_num=self.filter_num*2, alpha=0.8, kernel_size=3,
                                                activation=tf.nn.tanh)
                l1_H_x, l1_L_x = oct_conv_block(l1_H_x, l1_L_x, channel_num=self.filter_num*2, alpha=0.8, kernel_size=3,
                                                activation=tf.nn.tanh)
            ...
            with tf.variable_scope("layer_2", reuse=tf.AUTO_REUSE):
                ...
                l2_H_x, l2_L_x = oct_conv_block(l2_H_x, l2_L_x, channel_num=self.filter_num * 4, alpha=0.8,
                                                kernel_size=3,
                                                activation=tf.nn.tanh)
                l2_H_x, l2_L_x = oct_conv_block(l2_H_x, l2_L_x, channel_num=self.filter_num * 4, alpha=0.8,
                                                kernel_size=3,
            ...
            with tf.variable_scope("layer_3", reuse=tf.AUTO_REUSE):
                ...
                l3_H_x, l3_L_x = oct_conv_block(l3_H_x, l3_L_x, channel_num=self.filter_num * 6, alpha=0.8,
                                                kernel_size=3,
                                                activation=tf.nn.tanh)
                l3_H_x, l3_L_x = oct_conv_block(l3_H_x, l3_L_x, channel_num=self.filter_num * 6, alpha=0.8,
                                                kernel_size=3,
            ...
            with tf.variable_scope("layer_4", reuse=tf.AUTO_REUSE):
                ...
                l4_H_x, l4_L_x = oct_conv_block(l4_H_x, l4_L_x, channel_num=self.filter_num * 8, alpha=0.8,
                                                kernel_size=3,
                                                activation=tf.nn.tanh)
                l4_H_x, l4_L_x = oct_conv_block(l4_H_x, l4_L_x, channel_num=self.filter_num * 8, alpha=0.8,
                                                kernel_size=3,
                                                activation=tf.nn.tanh)
                l4_raw_output = oct_conv_final_layer(l4_H_x, l4_L_x, channel_num=self.filter_num * 8, kernel_size=3,
                                                     activation=tf.nn.tanh)
                img_shape = l4_raw_output.shape  # (32, 18, 32, 80)

                l4_output = tf.keras.layers.Conv2D((self.filter_num * 8), kernel_size=[img_shape.as_list()[1], img_shape.as_list()[2]], strides=1,kernel_initializer=tf.keras.initializers.glorot_normal())(l4_raw_output)
                l4_output = tf.reshape(l4_output, [-1,self.filter_num * 8])
    ...
    def actor_net(self, encoder_output):  # num_step = seq_size
        with tf.variable_scope("actor_net", reuse=tf.AUTO_REUSE):
            ...
            act_l1 = tf.keras.layers.Dense(108, kernel_initializer=tf.keras.initializers.glorot_normal(),
                                                        activation=tf.nn.tanh)(encoder_output)

            act_l1 = tf.keras.layers.Dense(64, kernel_initializer=tf.keras.initializers.glorot_normal())(act_l1)

            act_l1 = tf.keras.layers.Dense(32, kernel_initializer=tf.keras.initializers.glorot_normal(),
                                                        activation=tf.nn.tanh)(act_l1)
    ...
    def critic_net(self, encoder_output):  # num_step = seq_size
        with tf.variable_scope("critic_net", reuse=tf.AUTO_REUSE):
            ...
            critic_l1 = tf.keras.layers.Dense(108, kernel_initializer=tf.keras.initializers.glorot_normal(),
                                                        activation=tf.nn.tanh)(encoder_output)

            critic_l1 = tf.keras.layers.Dense(64, kernel_initializer=tf.keras.initializers.glorot_normal())(critic_l1)

            critic_l1 = tf.keras.layers.Dense(32, kernel_initializer=tf.keras.initializers.glorot_normal(),
                                                        activation=tf.nn.tanh)(critic_l1)
    ...
    def inverse_net(self,encoder_st,encoder_p1_st,actions_prob_holder):
        ...
        '''
        with tf.variable_scope("inverse_net", reuse=tf.AUTO_REUSE):
            print("encoder_st:",encoder_st)
            print("encoder_p1_st:",encoder_p1_st)

            inverse_l1 = tf.concat([encoder_st,encoder_p1_st],axis=-1)
            inverse_l2 = tf.keras.layers.Dense(self.filter_num*4, kernel_initializer=tf.keras.initializers.glorot_normal(),activation=tf.nn.tanh)(inverse_l1)
            inverse_l3 = tf.keras.layers.Dense(self.filter_num*6, kernel_initializer=tf.keras.initializers.glorot_normal())(inverse_l2)
            inverse_l4 = tf.keras.layers.Dense(self.filter_num*6, kernel_initializer=tf.keras.initializers.glorot_normal(),activation=tf.nn.tanh)(inverse_l3)
            inverse_logits =  tf.keras.layers.Dense(4, kernel_initializer=tf.keras.initializers.glorot_normal())(inverse_l4)
    ...
    def forward_net(self,encoder_st,encoder_p1_st,action_y_holder):
        ...
        with tf.variable_scope("forward_net", reuse=tf.AUTO_REUSE):
            ...
            forward_l2 = tf.keras.layers.Dense(self.filter_num*4, kernel_initializer=tf.keras.initializers.glorot_normal(),activation=tf.nn.tanh)(forward_l1)
            forward_l3 = tf.keras.layers.Dense(self.filter_num*6, kernel_initializer=tf.keras.initializers.glorot_normal())(forward_l2)
            forward_l4 = tf.keras.layers.Dense(self.filter_num*6, kernel_initializer=tf.keras.initializers.glorot_normal(),activation=tf.nn.tanh)(forward_l3)
            forward_p1_st = tf.keras.layers.Dense(self.filter_num*8, kernel_initializer=tf.keras.initializers.glorot_normal())(forward_l4)

...
class RL_model:
    ...
    def build_net(self):
        ...
        reconst_loss = tf.reduce_mean(tf.squared_difference(st_p1_holder, st_p1_predict), axis=[1, 2, 3])

        rt_loss = tf.reduce_mean(tf.squared_difference(rt_holder, rt_predict), axis=[1])

        VQ_loss = self.top_VQ_loss + self.bottom_VQ_loss
    ...
    def predict_next_reward_net(self, encoder_output):
        ...
        next_reward_out = tf.keras.layers.Conv2D(1, kernel_size=[next_reward_out_shape[1], next_reward_out_shape[2]],
                                                 ...
                                                 kernel_initializer=self.kernel, activation="sigmoid")(next_reward_out)
    ...
    def critic_net(self, encoder_output):  # num_step = seq_size
        ...
        V_value = tf.keras.layers.Conv2D(4, kernel_size=[logits_shape[1], logits_shape[2]],
                                         strides=1, padding="SAME",
